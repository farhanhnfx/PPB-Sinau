        ...
        builder.apply {
            if (note == null) {
                setPositiveButton("Tambah") {
                        dialog, which ->
                    val txtTitle = binding.editTitle.text.ifEmpty { "Judulss Default" }.toString()
                    val txtDesc = binding.editDesc.text.toString()
                    val txtAmount = binding.editAmount.text.ifEmpty { 0 }.toString().toInt()
                    val spnCategory = binding.spinnerCategory.selectedItem.toString()
                    activity.addNote(Note(title = txtTitle, description = txtDesc, category = spnCategory, amount = txtAmount))
                    dismiss()
                }
            }
            else {
                ...
                setPositiveButton("Edit") {
                    ...
                    val txtAmount = binding.editAmount.text.ifEmpty { 0 }.toString().toInt()
                    val spnCategory = binding.spinnerCategory.selectedItem.toString()
                    activity.updateNote(Note(id = note.id,title = txtTitle, description = txtDesc, category = spnCategory, amount = txtAmount))
                    dismiss()
                }
